...
void *worker_function(int n,BoundedBuffer& B,HistogramCollection* H,string file, FIFORequestChannel* C,string* fName)
{
    /*
...
// //	datamsg dm1 = datamsg(1,20.00,1);
	...
	while(( M =getopt(argc,argv,"p:n:w:f:b:")) != -1){
	  switch (M){
		  case 'p' :
			p = stoi(optarg);
			break;
	...
	vector <FIFORequestChannel*> ConsumersChan; //Store pointers to the Channels 
	...
	if(!fileRequest){
		...
		for(int i = 0; i < w ; i++){
			...
			FIFORequestChannel* B = new FIFORequestChannel(name, FIFORequestChannel::CLIENT_SIDE);
			ConsumersChan.push_back(B);
		
			thread temp(worker_function,a,ref(request_buffer),ref(H),Name,ConsumersChan[i],&Name);
